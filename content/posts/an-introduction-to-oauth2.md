---
title: "OAuth 2.0의 주요 개념과 인가 코드 흐름"
description: "OAuth 2.0에서 Resource Owner, Client, Authorization Server, Resource Server가 어떤 역할을 하는지 정리하고, 인가 코드 승인 흐름을 단계별로 설명합니다."
...
웹 서비스는 종종 다른 서비스의 기능이나 데이터를 연동합니다. 예를 들어 사진 인화 서비스가 사용자의 구글 포토 사진을 불러와 인화하는 기능을 생각해볼 수 있습니다. 이때 사용자가 구글 계정 아이디와 비밀번호를 사진 인화 서비스에 직접 넘기는 것은 위험합니다.

**OAuth (Open Authorization)**는 이런 문제를 해결하기 위해 등장한 **권한 부여(Authorization)를 위한 개방형 표준 프로토콜**입니다. OAuth를 사용하면 사용자는 비밀번호 같은 자격 증명을 제3자 애플리케이션에 노출하지 않고도, 특정 서비스의 리소스에 대한 접근 권한을 부여할 수 있습니다.

현재는 OAuth 1.0([RFC 5849](https://datatracker.ietf.org/doc/html/rfc5849))을 개선한 OAuth 2.0([RFC 6749](https://datatracker.ietf.org/doc/html/rfc6749))이 널리 사용됩니다. 이 글에서는 OAuth 2.0의 주요 개념과 인가 코드 흐름을 중심으로 설명합니다.

## OAuth 2.0의 주요 역할 (Roles)

OAuth 2.0의 흐름을 이해하려면 먼저 4가지 역할을 알아야 합니다.
...
이 글에서는 가장 대표적인 **인가 코드(Authorization Code)** 방식을 중심으로 설명합니다.
...
OAuth 2.0을 이해할 때는 토큰 자체보다 역할과 흐름을 먼저 잡는 것이 중요합니다. 특히 인가 코드 방식은 소셜 로그인과 외부 API 연동에서 자주 등장하므로, `client_id`, `redirect_uri`, `state`, 인가 코드, 액세스 토큰이 각각 어느 단계에서 쓰이는지 구분해두면 실제 구현을 따라가기 훨씬 쉽습니다.
