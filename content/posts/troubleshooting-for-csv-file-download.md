---
title: "CSV 다운로드 구현에서 자주 만나는 문제들"
description: "CSV 다운로드 기능을 만들 때 자주 겪는 엑셀 한글 깨짐, 프론트엔드 API 호출 시 파일 손상, 특수 문자로 인한 형식 오류의 원인과 해결 방법을 정리합니다."
...
이 글에서는 Spring Boot와 Kotlin 환경에서 CSV 다운로드 기능을 만들며 겪었던 세 가지 문제와 해결 과정을 정리합니다.
...
위처럼 직접 이스케이프 로직을 만들 수도 있지만, 모든 엣지 케이스를 직접 처리하기는 까다롭습니다. 가능하다면 **Apache Commons CSV** 나 **OpenCSV** 같은 CSV 라이브러리를 쓰는 편이 안전합니다. 이런 라이브러리는 RFC 4180 규칙을 이미 처리하고 있어, 직접 문자열을 조립할 때 생기는 실수를 줄일 수 있습니다.

## 체크리스트

CSV 다운로드 기능을 구현할 때는 다음 세 가지를 확인합니다.
...
이 세 가지만 지켜도 Excel, 브라우저, 프론트엔드 다운로드 코드 사이에서 생기는 흔한 문제를 대부분 피할 수 있습니다.
