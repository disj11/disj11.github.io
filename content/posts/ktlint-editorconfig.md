...
description: "Ktlint 규칙을 프로젝트에 맞게 조정하는 방법을 정리합니다. .editorconfig로 코드 스타일을 바꾸고, 특정 규칙을 비활성화하고, 규칙 ID를 찾는 방법을 다룹니다."
...
**Ktlint**는 Kotlin 코드 스타일을 검사하고 자동으로 포맷팅해주는 도구입니다. Ktlint의 기본 철학은 **"설정 없는(no configuration)"** 방식이고, 팀 안에서 코드 스타일 논쟁을 줄이는 데 도움이 됩니다.
...
이 글에서는 `.editorconfig` 파일로 Ktlint 동작을 프로젝트에 맞게 조정하는 방법을 정리합니다.
...
규칙을 비활성화하려면 정확한 규칙 ID를 알아야 합니다. 찾는 방법은 다음과 같습니다.

#### 방법 1: IntelliJ Ktlint 플러그인 사용하기

IntelliJ IDEA를 사용한다면 [Ktlint 플러그인](https://plugins.jetbrains.com/plugin/15057-ktlint)을 설치하는 방법이 편합니다. 플러그인을 설치하면 코드 스타일 위반 위치에 경고가 표시되고, 경고 메시지에서 규칙 ID를 확인할 수 있습니다.
...
팀원들이 커밋하기 전에 Ktlint 검사를 실행하도록 하면 코드 스타일을 더 일관되게 유지할 수 있습니다. `ktlint-gradle` 플러그인을 사용하는 경우 다음 명령어로 Git pre-commit hook을 설정할 수 있습니다.
...
Ktlint는 기본 설정을 그대로 쓸 때 가장 단순하지만, 프로젝트 사정에 따라 규칙을 조정해야 할 때도 있습니다. 이때 `.editorconfig`에 변경 이유와 규칙을 명확히 남겨두면, 코드 스타일 설정 자체가 팀의 합의 문서 역할을 할 수 있습니다.
