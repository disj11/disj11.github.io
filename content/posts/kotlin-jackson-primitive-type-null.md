...
description: "Kotlin 데이터 클래스와 Jackson을 함께 사용할 때 JSON에서 non-null 원시 타입(Long, Int) 필드가 누락되면 예외 대신 0이 들어갈 수 있습니다. 원인과 해결 방법을 정리합니다."
date: 2025-01-11T16:40:31+09:00
tags: ["kotlin", "jackson", "spring", "deserialization"]
---

## 들어가며: 예상과 다른 Jackson의 동작

Kotlin은 컴파일 시점에 null 안전성(Null Safety)을 보장합니다. 하지만 Jackson과 함께 사용할 때는 이 null 안전성이 기대와 다르게 동작하는 경우가 있습니다.
...
이 동작은 데이터가 누락되었는데도 요청이 성공한 것처럼 보이게 만들 수 있습니다. 이 글에서는 테스트로 문제를 재현하고, 원인과 해결 방법을 정리합니다.
...
-   **장점**: 한 번의 설정으로 애플리케이션 전체에 일관된 동작을 적용할 수 있습니다. 가장 직접적인 방법입니다.
...
Kotlin과 Jackson을 함께 쓸 때는 Kotlin 타입 시스템과 Jackson의 기본 역직렬화 규칙이 항상 같은 방향으로 움직이지 않는다는 점을 알아야 합니다. 필수 값이라면 전역 설정이나 `@JsonProperty(required = true)`로 누락을 막고, 선택 값이라면 nullable 타입으로 모델링하는 편이 명확합니다.
