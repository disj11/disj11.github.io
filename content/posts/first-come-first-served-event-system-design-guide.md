...
description: "평시 대비 15~30배 트래픽이 몰리는 선착순 이벤트에서 ALB LCU 예약, nginx 커넥션 한계, WAS 스레드 풀, Bulkhead/CircuitBreaker 연동, 봇 차단까지 운영 전에 확인할 항목을 정리합니다."
...
선착순(FCFS, First-Come-First-Served) 기능은 특정 시각에 트래픽이 한꺼번에 몰린다. 평시 대비 15~30배의 트래픽이 몇 초 안에 들어오기도 하고, 그 과정에서 평소에는 드러나지 않던 병목이 순서대로 터진다. 이 문서는 실제 운영에서 겪은 문제를 바탕으로, 선착순 기능을 열기 전에 확인해야 할 항목을 정리한 것이다.
...
AWS ALB(Application Load Balancer)는 트래픽에 따라 자동으로 스케일링한다. 다만 내부적으로 용량을 늘리는 데 시간이 걸리므로, **초당 50% 이상 급격히 증가하는 스파이크** 에서는 503 오류나 응답 지연이 먼저 발생할 수 있다.

**LCU(Load Balancer Capacity Unit)** 는 ALB가 처리하는 트래픽 양을 재는 단위다. 아래 4가지 차원 중 가장 높은 값이 과금과 용량 판단의 기준이 된다.
...
**Capacity Unit Reservation** 은 예상 피크 트래픽에 필요한 LCU를 이벤트 전에 미리 예약하는 기능이다. 예약된 용량은 바로 쓸 수 있는 상태로 유지되므로, 스파이크가 왔을 때 스케일링 지연을 줄일 수 있다.

### 문제

ALB의 자동 스케일링은 완만하게 증가하는 트래픽에는 잘 대응한다. 하지만 정각에 트래픽이 수직으로 치솟는 선착순 이벤트에서는 용량이 늘어나기 전에 요청이 먼저 버려질 수 있다.

### 해결

#### LCU 산정

예상 피크 RPS를 기준으로 필요한 LCU를 계산한다.
...
선착순 이벤트는 짧은 시간에 신규 연결이 많이 생기고 keep-alive 재사용률도 낮은 편이라, 보통 **신규 연결 기준** 이 병목이 된다.
...
- Pre-warming 요청 시 이벤트 날짜/시각, 예상 최대 RPS, 평균 응답 크기, 트래픽 증가 패턴을 함께 전달
...
애플리케이션 앞단의 리버스 프록시(nginx 등)에 `worker_connections` 같은 커넥션 제한이 기본값으로 남아 있으면, 스파이크 순간에 커넥션이 먼저 소진된다. 이 경우 요청은 애플리케이션 서버까지 도달하지 못한다.
...
WAS(Tomcat 등)의 스레드 풀을 기본값으로 둔 채 운영하면 스파이크 트래픽을 받아내기 어렵다. 스레드가 부족하면 요청이 큐에 쌓이고, 큐마저 가득 차면 요청이 거절된다. 코루틴이나 비동기 디스패처를 쓰는 경우에는 디스패처 지연이 메인 스레드까지 묶어둘 수 있다.

### 해결

- **스레드 풀 튜닝**: `max-threads`, `min-spare-threads`를 인스턴스 스펙과 예상 트래픽에 맞게 설정
- **타임아웃 설정**: 코루틴의 `withTimeout` 등으로 디스패처 지연 시 메인 스레드가 무기한 대기하지 않게 방어
...
| Tomcat 스레드 보호 | permit 초과 시 즉시 실패 → 보호 | Tomcat 스레드를 소비하지 않음 → 더 유리 |
...
이때 기준은 단순하다. **과부하 상태에서는 트래픽을 더 만들지 않는 방향** 으로, **실제 장애에서는 재시도를 안내하는 방향** 으로 UX를 나눈다.

외부 호출이 실패할 때 무조건 예외를 던져 5xx를 반환할 필요도 없다. graceful degradation(빈 리스트 반환, 기본값 사용 등)이 가능한지 먼저 봐야 한다. 페이지의 핵심이 아닌 보조 데이터(추천, 배너 등)는 외부 호출이 실패해도 빈 상태로 렌더링하는 편이 사용자에게 덜 거칠 수 있다.
