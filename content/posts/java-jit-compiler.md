---
title: "Java JIT 컴파일러의 동작 원리와 성능 영향"
description: "JIT(Just-In-Time) 컴파일러가 자바 바이트코드를 어떻게 최적화하는지, 계층적 컴파일(Tiered Compilation)은 어떻게 동작하는지 예제와 함께 정리합니다."
date: 2023-05-10T23:56:05+09:00
lastmod: 2024-12-29T19:45:00+09:00
tags: ["java", "jvm", "performance"]
---

## 들어가며: JVM은 언제 코드를 컴파일할까?
...
하지만 인터프리터 방식은 코드를 실행할 때마다 번역 과정을 거쳐야 하므로, 기계어로 직접 컴파일되어 실행되는 네이티브 언어(C, C++ 등)에 비해 느릴 수 있습니다. 이 문제를 줄이기 위해 현대 JVM은 **JIT(Just-In-Time) 컴파일러** 를 사용합니다.

JIT 컴파일러는 프로그램 실행 중에 자주 사용되는 '뜨거운(hot)' 코드를 감지하고, 해당 부분을 네이티브 기계어로 컴파일합니다. 이렇게 컴파일된 코드는 다음 실행부터 인터프리터를 거치지 않고 실행됩니다. 이 글에서는 JIT 컴파일러의 동작 원리와 계층적 컴파일을 정리합니다.
...
-   **C2 (Server Compiler)**: C1보다 컴파일하는 데 시간이 더 오래 걸리지만, 더 적극적인 최적화를 수행합니다. 장시간 실행되는 서버 애플리케이션의 최대 성능에 중점을 둡니다.
...
이 방식을 통해 JVM은 애플리케이션의 시작 시간과 장기 실행 성능 사이의 균형을 맞춥니다. 처음에는 인터프리터로 빠르게 시작하고, 자주 실행되는 코드는 점진적으로 더 높은 수준의 컴파일을 적용합니다.
...
결과를 보면 JIT 컴파일러가 활성화되었을 때 **약 20배 이상** 빠르게 실행되었습니다. 반복적으로 실행되는 코드에서는 JIT 최적화가 성능에 큰 영향을 줄 수 있습니다.

> **주의: IntelliJ 사용자를 위한 팁**
...
JIT 컴파일러는 JVM이 실행 중인 코드를 관찰하고 자주 실행되는 부분을 점진적으로 최적화하는 장치입니다. 성능 문제를 분석할 때는 코드 자체뿐 아니라 워밍업, 계층적 컴파일, 실행 옵션까지 함께 봐야 합니다.
