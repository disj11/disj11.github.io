...
description: "MySQL 다중 컬럼 인덱스에서 컬럼 순서가 왜 중요한지, 범위 조건(>, <, BETWEEN, LIKE)이 인덱스 사용에 어떤 영향을 주는지 예제로 정리합니다."
...
데이터베이스 성능 튜닝에서 **인덱스** 는 가장 먼저 떠올리는 도구입니다. 특히 여러 컬럼을 묶어 만드는 다중 컬럼 인덱스(Multi-column Index)는 잘 맞으면 효과가 크지만, 컬럼 순서와 조건 형태를 잘못 잡으면 기대만큼 동작하지 않습니다. 많은 개발자가 다음 질문에 부딪힙니다.
...
이 질문의 답은 **다중 컬럼 인덱스의 자료 구조** 와 **MySQL 옵티마이저의 동작 방식** 에 있습니다. 이 글에서는 컬럼 순서와 범위 조건이 인덱스 사용에 어떤 영향을 주는지 정리합니다.
...
MySQL 옵티마이저는 `WHERE` 절의 조건을 보고 인덱스를 어떻게 사용할지 결정합니다. 여기서는 **탐색(Seek)** 과 **스캔(Scan)** 의 차이를 이해해야 합니다.
...
### Case 2: 범위 조건(>, <, BETWEEN, LIKE)이 포함된 경우
...
이 원칙을 기억하고 인덱스를 설계하면, 왜 어떤 조건은 인덱스를 잘 타고 어떤 조건은 필터링으로 밀리는지 훨씬 쉽게 판단할 수 있습니다.
